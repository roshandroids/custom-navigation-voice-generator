...
/// Moves to the previous instruction in the recording workflow.
class GetPreviousInstruction {
  const GetPreviousInstruction(this._repository);
  ...
  Future<Result<Instruction>> execute({
    ...
    if (index <= 0) {
      return const Err(NotFoundFailure('No previous instruction.'));
    }
