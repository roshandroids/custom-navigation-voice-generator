...
| Voice license (en_US-joe-medium) | **Dataset CC0** (OHF-Voice/voice-datasets). 1 speaker, 22,050 Hz. Finetuned from U.S. English lessac. Most permissive English option — selected for the hybrid experiment. | https://huggingface.co/rhasspy/piper-voices/blob/main/en/en_US/joe/medium/MODEL_CARD |
...
| Dataset restrictions | chitwan + joe: **CC0**. google-medium / google-x_low: **CC-BY-SA-4.0** (share-alike). | per-voice MODEL_CARDs (above) |
| Commercial use       | GPL-3.0 code: copyleft obligations for distribution. Voice models: chitwan + joe **CC0**; google voices **CC-BY-SA-4.0** (share-alike). **No legal conclusion.** | — |
