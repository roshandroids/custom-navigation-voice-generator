...
- **Piper** — fast local neural TTS (VITS), ONNX-based, per-language voice models. Has identifiable Nepali voices (`ne_NP-chitwan-medium`, `ne_NP-google-x_low`, `ne_NP-google-medium`) and English voices (`en_US-joe-medium` baseline). Nepali and English voices verified working standalone ([[piper-nepali-voices]], [[piper-english-benchmark]]).
